...
```
Tip：（1）当前目录会显示在上方
（2）注意Git中复制为`ctrl+insert`，粘贴为`shift+insert`
...
```
Tip：这会将add方法创建的文件和commit的说明提交到github仓库
...
```
Tip：<mark style="background: #7DFFFAA6;">工作目录指git克隆到本地的可见的，用于修改的目录。本地仓库指在项目根目录下的隐藏文件夹.git</mark>。
...
```
Tip：（1）快进合并：如果当前分支完全落后于目标分支，没有分叉点时，Git 会直接将当前分支移动到目标分支的位置。<mark style="background: #7DFFFAA6;">假设将A1分支合并到main后，使用git merge main可以将A1快进到main的当前位置</mark>。
...
```
Tip：（1）rebase本质上是提取源分支的提交，并逐个应用到目标分支上，也可以理解为在目标分支上"重演"。<mark style="background: #A3FFB8A6;">因此目标分支的修改不会缺失</mark>。
（2）注意这项命令会改变历史，测试时请使用安全的Merge。
...
```
Tip：可以采取init或者clone来初始化一个仓库。通过clone不需要查看配置
